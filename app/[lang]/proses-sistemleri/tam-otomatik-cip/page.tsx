...
import { Download, CheckCircle, ChevronRight, FileText, ShieldCheck, Activity, Settings, Droplets, Repeat, Gauge } from 'lucide-react';
import { getDictionary } from '../../../../dictionaries/getDictionary';
import { getLocalizedUrl } from '../../../../dictionaries/routes';

export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  const resolvedParams = await params;
  const dict = await getDictionary(resolvedParams.lang, 'cipSystems');
  const t = dict.cipSystemsPage;
...
export default async function CIPSystemsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const dict = await getDictionary(lang, 'cipSystems');
  const t = dict.cipSystemsPage;

  const jsonLd = {
    "@context": "https://schema.org", "@type": "Product",
    "name": t.hero.title,
    "image": "https://www.welltech.com.tr/assets/images/proses-sistemleri/kategori-cip.webp",
    ...
    <div className="bg-gray-50 pb-24 font-sans selection:bg-[#E35205] selection:text-white">
      ...
      <section className="relative h-[65vh] pt-32 pb-20 px-6 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src="/assets/images/proses-sistemleri/kategori-cip.webp" alt={t.hero.title} fill priority className="object-cover" sizes="100vw" />
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="relative h-64 sm:h-80 w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                <Image src="/assets/images/proses-sistemleri/cip/proses.webp" alt={t.technology.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
            </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 overflow-hidden">
              <div className="grid md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      ...
                      { text: t.features.items[1], icon: <Droplets className="w-5 h-5 text-[#005284]" /> },
                      { text: t.features.items[2], icon: <Activity className="w-5 h-5 text-[#E35205]" /> },
                      { text: t.features.items[3], icon: <Settings className="w-5 h-5 text-[#005284]" /> },
                      { text: t.features.items[4], icon: <ShieldCheck className="w-5 h-5 text-[#E35205]" /> },
                      { text: t.features.items[5], icon: <FileText className="w-5 h-5 text-[#005284]" /> },
                    ].map((f, idx) => (
                ...
                <div className="relative h-48 md:h-full w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                  <Image src="/assets/images/proses-sistemleri/cip/otomasyon.webp" alt={t.features.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-3 gap-8 items-center">
              <div className="relative h-64 sm:h-96 w-full overflow-hidden border border-gray-100 rounded-lg shadow-sm group">
                <Image src="/assets/images/proses-sistemleri/cip/cizim.webp" alt={t.table.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
              </div>
              <div className="md:col-span-2 overflow-x-auto">
                ...
                <table className="w-full text-left border-collapse min-w-[300px]">
                  ...
                  <tbody className="text-sm text-gray-700">
                    {t.table.rows.map((row: { config: string; tanks: string; sector: string; highlight: string }, i: number) => (
                      <tr key={i} className="border-b hover:bg-gray-50 transition-colors">
                        <td className={`p-4 font-bold ${row.highlight === 'orange' ? 'text-[#E35205]' : row.highlight === 'dark' ? 'text-gray-900' : 'text-[#005284]'}`}>{row.config}</td>
                        <td className="p-4">{row.tanks}</td>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {t.gallery.items.map((item: { src: string; label: string; alt: string }, index: number) => (
                  <Link href={`/${lang}/referanslar`} key={index} className="block relative h-64 sm:h-80 w-full overflow-hidden rounded-xl shadow-md group cursor-pointer">
                    <Image src={`/assets/images/proses-sistemleri/cip/${item.src}`} alt={item.alt} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 25vw" />
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 space-y-3">
                ...
                <div className="relative h-40 w-full overflow-hidden border border-gray-100 rounded shadow-inner group">
                  <Image src="/assets/images/proses-sistemleri/cip/referans.webp" alt={t.sidebar.reference.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
              ...
              <div className="bg-gray-900 p-6 rounded-xl shadow-lg text-white">
                ...
                <Link href={`/${lang}/iletisim`} className="w-full flex items-center justify-center gap-2 bg-[#E35205] text-white px-4 py-3 rounded text-sm font-bold tracking-wider hover:bg-white hover:text-[#E35205] transition-colors shadow-md">
                  {t.sidebar.cta.btn}
                </Link>
              </div>
