...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const dict = await getDictionary(resolvedParams.lang, 'diaphragmPumps');
  const t = dict.diaphragmPumpsPage;
  return {
...
export default async function DiaphragmPumpsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const dict = await getDictionary(lang, 'diaphragmPumps');
  const t = dict.diaphragmPumpsPage;
  ...
  const jsonLd = {
    ...
    "@type": "Product",
    "name": t.hero.title,
    "image": "https://www.welltech.com.tr/assets/images/pompalar/havali-diyafram/banner.webp",
    "description": t.metadata.description,
    ...
    <div className="bg-gray-50 pb-24 selection:bg-[#E35205] selection:text-white">
      ...
      <section className="relative h-[65vh] pt-32 pb-20 px-6 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src="/assets/images/pompalar/havali-diyafram/banner.webp" alt={t.hero.title} fill priority className="object-cover transition-transform duration-[3000ms] scale-105" sizes="100vw" />
        ...
        <div className="max-w-7xl mx-auto relative z-10 h-full flex flex-col justify-center">
          <div className="flex items-center gap-2 text-[10px] md:text-xs font-bold tracking-widest text-blue-200 mb-6">
            ...
            <Link href={getLocalizedUrl('pompalar', lang)} className="hover:text-white transition-colors">{t.breadcrumb.pumps}</Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-gray-600 leading-relaxed text-sm mb-6">{t.safety.desc}</p>
              </div>
              <div className="relative h-64 sm:h-80 w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                <Image src="/assets/images/pompalar/havali-diyafram/detay.webp" alt={t.safety.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
            </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 overflow-hidden">
              <div className="grid md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      ...
                      { text: t.features.items[3], icon: <Activity className="w-5 h-5 text-[#005284]" /> },
                ...
                <div className="relative h-48 md:h-full w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                  <Image src="/assets/images/pompalar/havali-diyafram/atex.webp" alt={t.features.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-3 gap-8 items-center">
              <div className="relative h-64 sm:h-96 w-full overflow-hidden border border-gray-100 rounded-lg shadow-sm group">
                <Image src="/assets/images/pompalar/havali-diyafram/cizim.webp" alt={t.table.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
              </div>
              <div className="md:col-span-2 overflow-x-auto">
                ...
                <table className="w-full text-left border-collapse min-w-[300px]">
                  ...
                  <tbody className="text-sm text-gray-700">
                    {t.table.rows.map((row: { size: string; flow: string; pressure: string; particle: string; highlight: string }, i: number) => (
                      <tr key={i} className="border-b hover:bg-gray-50 transition-colors">
                        <td className={`p-4 font-bold ${row.highlight === 'orange' ? 'text-[#E35205]' : row.highlight === 'dark' ? 'text-gray-900' : 'text-[#005284]'}`}>{row.size}</td>
                        <td className="p-4">{row.flow}</td>
                        <td className="p-4">{row.pressure}</td>
                        <td className="p-4">{row.particle}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {t.gallery.items.map((item: { src: string; label: string; alt: string }, index: number) => (
                  <Link href={`/${lang}/referanslar`} key={index} className="block relative h-64 sm:h-80 w-full overflow-hidden rounded-xl shadow-md group cursor-pointer">
                    <Image src={`/assets/images/pompalar/havali-diyafram/${item.src}`} alt={item.alt} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 25vw" />
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 space-y-3">
                ...
                <div className="relative h-40 w-full overflow-hidden border border-gray-100 rounded shadow-inner group">
                  <Image src="/assets/images/pompalar/havali-diyafram/referans.webp" alt={t.sidebar.reference.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
