...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const dict = await getDictionary(resolvedParams.lang, 'selfPrimingPumps');
  const t = dict.selfPrimingPumpsPage;
  return {
...
export default async function SelfPrimingPumpsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const dict = await getDictionary(lang, 'selfPrimingPumps');
  const t = dict.selfPrimingPumpsPage;
  ...
  const jsonLd = {
    ...
    "@type": "Product",
    "name": t.hero.title,
    "image": "https://www.welltech.com.tr/assets/images/pompalar/kendinden-emisli/banner.webp",
    "description": t.metadata.description,
    ...
    <div className="bg-gray-50 pb-24 selection:bg-[#E35205] selection:text-white">
      ...
      <section className="relative h-[65vh] pt-32 pb-20 px-6 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src="/assets/images/pompalar/kendinden-emisli/banner.webp" alt={t.hero.title} fill priority className="object-cover" sizes="100vw" />
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="relative h-64 sm:h-80 w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                <Image src="/assets/images/pompalar/kendinden-emisli/fan.webp" alt={t.technology.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
            </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      { text: t.features.items[0], icon: <ArrowUpFromLine className="w-5 h-5 text-[#E35205]" /> },
                      { text: t.features.items[1], icon: <ShieldCheck className="w-5 h-5 text-[#005284]" /> },
                      { text: t.features.items[2], icon: <Wind className="w-5 h-5 text-[#E35205]" /> },
                      { text: t.features.items[3], icon: <RefreshCw className="w-5 h-5 text-[#005284]" /> },
                      { text: t.features.items[4], icon: <Activity className="w-5 h-5 text-[#E35205]" /> },
                ...
                <div className="relative h-48 md:h-full w-full overflow-hidden border border-gray-100 rounded-lg shadow-inner group">
                  <Image src="/assets/images/pompalar/kendinden-emisli/motor.webp" alt={t.features.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-100 grid md:grid-cols-3 gap-8 items-center">
              <div className="relative h-64 sm:h-96 w-full overflow-hidden border border-gray-100 rounded-lg shadow-sm group">
                <Image src="/assets/images/pompalar/kendinden-emisli/cizim.webp" alt={t.table.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {t.gallery.items.map((item: { src: string; label: string; alt: string }, index: number) => (
                  <Link href={`/${lang}/referanslar`} key={index} className="block relative h-64 sm:h-80 w-full overflow-hidden rounded-xl shadow-md group cursor-pointer">
                    <Image src={`/assets/images/pompalar/kendinden-emisli/${item.src}`} alt={item.alt} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 25vw" />
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 space-y-3">
                ...
                <div className="relative h-40 w-full overflow-hidden border border-gray-100 rounded shadow-inner group">
                  <Image src="/assets/images/pompalar/kendinden-emisli/referans.webp" alt={t.sidebar.reference.imgAlt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
